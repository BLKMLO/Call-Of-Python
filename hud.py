...
class HUD:
    ...
    def _draw_slots(self, screen, player, box=44):
        ...
        for slot in range(len(WEAPON_ORDER)):
            ...
            num = self._text(self.font, str(slot + 1), border)
            screen.blit(num, (x + 4, y + 2))
            if weapon is not None:
                icon_key = (weapon.spec.id, box)
                icon = self._slot_icon_cache.get(icon_key)
                if icon is None:
                    icon = pygame.transform.scale(
                        assets.get("pickup_" + weapon.spec.id),
                        (box - 10, box - 10))
                    self._slot_icon_cache[icon_key] = icon
    ...
    def _draw_level_label(self, screen, level):
        ...
        label = self._text(self.font, text.upper(), HUD_TEXT)
    ...
    def _draw_minimap(self, screen, player, enemies, level, pickups=(), scale=6):
        ...
        # La base (statique) est blittée directement ; seuls les points
        # mobiles sont dessinés par-dessus, à l'offset de la carte.
        frame = pygame.Rect(8, 8, self._minimap_base.get_width() + 4,
                            self._minimap_base.get_height() + 4)
        pygame.draw.rect(screen, (4, 10, 15), frame)
        screen.blit(self._minimap_base, (10, 10))
        for pickup in pickups:
            # Les packs de vie cachés ne trahissent pas leur position ici.
            if not pickup.taken and not pickup.hidden:
                pygame.draw.circle(screen, (240, 210, 90),
                                   (int(pickup.x * scale) + 10,
                                    int(pickup.y * scale) + 10), 2)
        for enemy in enemies:
            if enemy.alive:
                radius = 3 if enemy.IS_BOSS else 2
                pygame.draw.circle(screen, (230, 70, 60),
                                   (int(enemy.x * scale) + 10,
                                    int(enemy.y * scale) + 10), radius)
        px, py = int(player.x * scale) + 10, int(player.y * scale) + 10
        pygame.draw.circle(screen, (90, 220, 90), (px, py), 3)
        tip = (px + int(math.cos(player.angle) * scale * 1.5),
               py + int(math.sin(player.angle) * scale * 1.5))
        pygame.draw.line(screen, (90, 220, 90), (px, py), tip, 1)
