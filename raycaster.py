...
def zoom_screen(screen, zoom):
    ...
    global _zoom_scratch
    if zoom <= 1.001:
        return
    w, h = screen.get_size()
    cw, ch = int(w / zoom), int(h / zoom)
    # Tampon unique réutilisé : en visée soutenue, plus aucune allocation
    # plein format par frame (réalloué seulement si la taille change).
    if _zoom_scratch is None or _zoom_scratch.get_size() != (cw, ch):
        _zoom_scratch = pygame.Surface((cw, ch))
    _zoom_scratch.blit(screen, (0, 0), ((w - cw) // 2, (h - ch) // 2, cw, ch))
...
class Raycaster:
    ...
    def set_level(self, level):
        ...
        # Caractère de repli pour une case sans texture connue : calculé une
        # fois ici plutôt qu'à chaque colonne rendue.
        self._default_char = next(iter(self.tex_cols))
    ...
    def _draw_wall_column(self, screen, x, depth, tile, vertical, offset):
        ...
        if tile == self.energy_tile:
            ...
            if fade <= 0.02:
                return
            # Quantifié par pas de 16 : quelques variantes par colonne,
            # réutilisées via le cache au lieu d'une copie par frame.
            alpha = min(255, int(min(1.0, fade) * 16) * 16)

        if h_mult == 1.0:
            column = (self.tex_cols.get(tile)
                      or self.tex_cols[self._default_char])[shade][tx]
    ...
    def _draw_column(self, screen, x, column, native_h, top, total_h,
        ...
        if top >= 0 and top + total_h <= scr_h:
            ...
            if alpha < 255:
                # Variante estompée mémoïsée sous une clé dédiée : la
                # surface partagée du chemin opaque n'est jamais mutée.
                cache_key = (*cache_key, alpha)
        ...
        if alpha < 255:
            seg.set_alpha(alpha)   # surface fraîche, non partagée
